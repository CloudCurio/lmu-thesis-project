...
output_dir <- paste("..//..//HMM outputs//Alleloscope_500kb_300nSNP//", "2_state_model//", sep = "")
...
for (model in names(models)){
  ...
  metrics_list <- list("by_Class" = per_class_metrics_df, "overall" = overall_metrics_df)
  ...
  for (cell in unique(models[[model]][["data"]]$cell)){
    ...
    base_class <- which(response_mtx[,"Re1.(Intercept)"] == min(response_mtx[,"Re1.(Intercept)"]))
    ...
    insert_df$HMM_pred <- ifelse(pred == gain_class, 3, 2)
    ...
    metrics_list[["by_Class"]][cell, ] <- extracted_metrics
...
for (model in names(models)){
  ...
  corr_df <- data.frame(matrix(ncol = 15, nrow = 2))
  rownames(corr_df) <- c("base", "gain")
  
  print(paste("Correlation between metrics and missing theta-values for the", model, "model"))
  metrics_merged <- models[[model]][["metrics"]][["by_Class"]]
  metrics_merged <- cbind(models[[model]][["metrics"]][["by_Class"]], models[[model]][["metrics"]][["overall"]])
...
for (metric in colnames(per_class_metrics_df)){
  for (model in names(models)){
    ...
    plot_data <- models[[model]][["metrics"]][["by_Class"]][,metric]
...
for (metric in colnames(per_class_metrics_df)){
  ...
  for (model in names(models)){
    extracted_metrics[[model]] <- models[[model]][["metrics"]][["by_Class"]][, metric] 
  }
  value_column <- c()
...
epiA_truth <- ifelse(cnv_labels_pb$wgs_score >= 2.5, 3, ifelse(cnv_labels_pb$wgs_score <= 1.5, 1, 2))
...
# rownames(epiA_eval_df) <- c("base", "gain")
...
#   extracted_metrics <- as.data.frame(t(confmat_epiA$byClass[which(c("base", "gain") == cnv_class),]))
#   extracted_metrics$overall_Acc <- confmat_epiA$overall[["Accuracy"]]
#   
#   #build ROC curves
#   binary_ref <- ifelse(epiA_truth == "gain", 1, 0)
#   binary_pred <- ifelse(epiA_pred == "gain", 1, 0)
...
#   for (cnv_type in c("base", "gain")){
#     #subset the data by cnv type and model name
#     data_sub <- models[[model]][["metrics"]][["by_Class"]]
...
#   do.call(grid.arrange, c(plot_list[[metric]], ncol = 2))
# }
# #plot overall_Acc
...
for (model in names(models)){
  ...
  if (1 %in% HMM_preds$WGS_score){
    color_set <- c("#2171B5", "#1fb424", "#b4421f")
    names(color_set) <- c("loss", "base", "gain")
  } else {
    color_set <- c("#1fb424", "#b4421f")
    names(color_set) <- c("base", "gain")
  }
