...
#' @import rtracklayer
#' @export
estimate=function(Obj_filtered=NULL,max_nSNP=30000, plot_stat=TRUE, min_cell=5, min_snp=0, rds_path=NULL, cont=FALSE, max_iter=50, phases=NULL, sub_cells=NULL, sub_region=NULL){
    ...
    for(chrr in as.character(selseg)){
      ...
      if(! chk %in% list.files(rds_path)){
        ...
        if(is.null(phases)){
          ...
          result=EM_debug(ref_table = as.matrix(total_all_sub-alt_all_sub), alt_table = as.matrix(alt_all_sub) ,seed=1000, max_iter=max_iter)
